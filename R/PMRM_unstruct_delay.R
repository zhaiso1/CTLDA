...
#' @export
#'
unstruct.delay.m12 <- function(dat){
  months <- unique(dat$M)
  ...
  start_vec <- c(y.pbo.x, b = rep(0, length(months)-1))

  t_pmrm <- tryCatch({
    gnls(model = y ~ f.unstruct.delay.m12(M, v0, v1, v2, v3, v4, b1, b2, b3, b4),
         data = dat,
