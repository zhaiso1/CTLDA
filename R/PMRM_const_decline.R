...
#' @export
#'
const.decline.m12 <- function(dat){
  months <- unique(dat$M)
  ...
  b_init <- mean(y.pbo.x[-1] - y.trt.x[-1])
  ...
  # fit model
  pd_pmrm <- tryCatch({
    gnls(model = y ~ f.const.decline.m12(M, v0, v1, v2, v3, v4, b),
         data = dat,
...
f.const.decline.m12 <- function(t, v0, v1, v2, v3, v4, b) {
  months <- seq(0, 12, 3)

  v <- c(v0[1], v1[1], v2[1], v3[1], v4[1])
  bx <- cbind(0, b, b, b, b)

  v[match(t, months)] - bx[cbind(1:length(t), match(t, months))]
}
